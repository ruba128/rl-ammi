"""Source: rlkit/rlkit/torch/distributions.py"""
...
# import rlkit.th.pytorch_util as ptu
...
class TorchDistributionWrapper(Distribution):
    ...
    def sample(self, sample_size=th.Size()):
        return self.distribution.sample(sample_shape=sample_size)

    def rsample(self, sample_size=th.Size()):
...
@th.distributions.kl.register_kl(TorchDistributionWrapper,
...
@th.distributions.kl.register_kl(TorchIndependent, TorchIndependent)
...
class GaussianMixture(Distribution):
    ...
    def log_prob(self, value, ):
        ...
        log_p = th.stack(log_p, -1)
        log_p = log_p.sum(dim=1)
        log_weights = th.log(self.weights[:, :, 0])
        lp = log_weights + log_p
        m = lp.max(dim=1)[0]  # log-sum-exp numerical stability trick
        log_p_mixture = m + th.log(th.exp(lp - m).sum(dim=1))
    ...
    def mle_estimate(self):
        ...
        ind = th.argmax(self.weights, dim=1) # [:, 0]
        c.scatter_(1, ind, 1)
        s = th.matmul(self.normal_means, c[:, :, None])
        return th.squeeze(s, 2)
...
class GaussianMixtureFull(Distribution):
    ...
    def log_prob(self, value, ):
        log_p = [self.normals[i].log_prob(value) for i in range(self.num_gaussians)]
        log_p = th.stack(log_p, -1)
        log_weights = th.log(self.weights)
        lp = log_weights + log_p
        m = lp.max(dim=2, keepdim=True)[0]  # log-sum-exp numerical stability trick
        log_p_mixture = m + th.log(th.exp(lp - m).sum(dim=2, keepdim=True))
        raise NotImplementedError("from Vitchyr: idk what the point is of "
                                  ...
                                  "interface.")
        return th.squeeze(log_p_mixture, 2)

    def sample(self):
        z = self.normal.sample().detach()
        c = self.categorical.sample()[:, :, None]
        s = th.gather(z, dim=2, index=c)
        return s[:, :, 0]
    ...
    def mle_estimate(self):
        ...
        This often computes the mode of the distribution, but not always.
        """
        ind = th.argmax(self.weights, dim=2)[:, :, None]
        means = th.gather(self.normal_means, dim=2, index=ind)
        return th.squeeze(means, 2)
...
class TanhNormal(Distribution):
    ...
    """
    def __init__(self, normal_mean, normal_std, epsilon=1e-6, device='cpu'):
        """
    ...
    def _log_prob_from_pre_tanh(self, pre_tanh_value):
        ...
        correction = - 2. * (
            th.log(th.Tensor([2.]).to(self.device))
            - pre_tanh_value
            - th.nn.functional.softplus(-2. * pre_tanh_value)
        ).sum(dim=1)
    ...
    def log_prob(self, value, pre_tanh_value=None):
        if pre_tanh_value is None:
            ...
            value = th.clamp(value, -0.999999, 0.999999)
            pre_tanh_value = th.log(1+value) / 2 - th.log(1-value) / 2
    ...
    def rsample_with_pretanh(self):
        z = (
                ...
                self.normal_std *
                MultivariateDiagonalNormal(
                    th.zeros(self.normal_mean.size()).to(self.device),
                    th.ones(self.normal_std.size()).to(self.device)
                ).sample()
        )
        return th.tanh(z), z
    ...
    @property
    def mean(self):
        return th.tanh(self.normal_mean)
    ...
    def get_diagnostics(self):
        ...
        stats.update(create_stats_ordered_dict(
            'normal/log_std',
            ptu.get_numpy(th.log(self.normal_std)),
        ))
        return stats
...
#     #             (1+value) / (1-value)
#     #         ) / 2
#     #     return self.normal.log_prob(pre_tanh_value) - th.log(1 - value * value + self.epsilon)
...
#         return log_prob + correction.reshape(-1,1)
